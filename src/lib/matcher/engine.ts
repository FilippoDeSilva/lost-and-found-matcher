...
export async function calculateReportMatchAsync(
  ...
  if (!isNaN(lostTime) && !isNaN(foundTime)) {
    ...
    reasons.push(`Date comparison estimated`);
  }
...
export function calculateReportMatch(
  ...
  let textScore = 0;
  const sharedColors = lostConcepts.colorGroups.filter(c => foundConcepts.colorGroups.includes(c));
  if (sharedColors.length > 0) textScore += 35;
  ...
  if (!isNaN(lostTime) && !isNaN(foundTime)) {
    const diffHours = (foundTime - lostTime) / (1000 * 60 * 60);
    if (diffHours >= 0 && diffHours <= 24) timeScore = 100;
    else if (diffHours > 24 && diffHours <= 72) timeScore = 85;
    else if (diffHours > 72 && diffHours <= 168) timeScore = 60;
    else if (diffHours > 168 && diffHours <= 336) timeScore = 40;
    else if (diffHours > 336) timeScore = 25;
    else timeScore = Math.abs(diffHours) <= 24 ? 40 : 0;
  }
  ...
  const reasons = [
    `Matching category: ${lostReport.category}`,
    locResult.reason,
    `Temporal relevance score: ${timeScore}%`
  ];

  if (conceptMatchedName) reasons.push(`Identified common item concept: "${conceptMatchedName}"`);
  if (sharedColors.length > 0) reasons.push(`Matching color profile: ${sharedColors.join(', ').replace('_', ' ')}`);
